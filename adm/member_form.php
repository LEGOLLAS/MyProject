...
<div class="tbl_frm01 tbl_wrap">
    ...
    <tr>
        ...
        <td><input type="hidden" name="mb_nick" value="<?php echo $mb['mb_nick'] ?>" id="mb_nick"><!--<input type="text" name="mb_nick" value="<?php echo $mb['mb_nick'] ?>" id="mb_nick" required class="required frm_input" size="15" minlength="2" maxlength="20">--></td>
    </tr>
    <tr>
...
?>
    <?php for ($i=5; $i<=10; $i++) { if($i>5) { continue; } ?>
    <tr>
        <th scope="row"><label for="mb_<?php echo $i ?>"><?php echo $f_name[$i]; ?></label></th>
        <?php if ($i == 2) { ?>
        <td colspan="3">
            <select name="mb_2">
            <?php foreach ($group_list as $key=>$value) { ?>
                <option value="<?php echo $key; ?>"<?php if($mb['mb_2'] == $key) { echo ' selected'; } ?>><?php echo $value; ?></option>
            <?php } ?>
            </select>
        </td>
        <?php } else { ?>
