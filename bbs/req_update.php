...
if ($program_data['status'] != 'A') {
    alert("정상적인 접근이 아닙니다.", G5_URL);
}
if ($program_data['status2'] != 'S' && !($program_data['status2'] == 'D' && $w == 'u')) {
    alert("정상적인 접근이 아닙니다.", G5_URL);
}
if ($member['mb_no'] != $program_data['member_id']) {
    alert("정상적인 접근이 아닙니다.", G5_URL);
}

$_POST['wr_subject'] = '장학금 신청서';
$_POST['wr_content'] = '.';
// wr_1 : 프로그램 신청 id
$_POST['wr_2'] = $program_data['point'] * 10000; //금액
$_POST['wr_3'] = '20'.$_POST['year'].' 년도 제 '.$_POST['hk'].' 학기'; //해당년도
$_POST['wr_4'] = $program_data['m_group'].' / '.$member['mb_3']; //대학/학과
$_POST['wr_5'] = $member['mb_4'];; //학년
$_POST['wr_6'] = $member['mb_name']; //성명
$_POST['wr_7'] = $member['mb_1']; //학번
$_POST['wr_8'] = $member['mb_hp']; //연락처
$_POST['wr_9'] = $_POST['year'];
$_POST['wr_10'] = $_POST['hk'];
...
if(!$_POST['year']) {
    $msg[] = '<strong>핵당년도</strong>를 입력하세요';
}
if(!$_POST['hk']) {
    $msg[] = '<strong>핵당학기</strong>를 입력하세요';
}
...
$sql = "update program_req set status2='D', scholarship_id='".$wr_id."' where program_id='".$_POST['wr_1']."' and member_id='".$member['mb_no']."'";
sql_query($sql);

delete_cache_latest($bo_table);

if ($file_upload_msg)
    alert($file_upload_msg, G5_HTTP_BBS_URL.'/req.php?wr_id='.$wr_id.'&amp;w=u&amp;id='.$_POST['wr_1']);
else
    goto_url(G5_HTTP_BBS_URL.'/mypage.php');
?>
