...
$sql = "SELECT * FROM program_req WHERE id='".$req_id."'";
...
if ($req_info['status'] != 'A') {
    alert("정상적인 접근이 아닙니다.", G5_URL);
}
if ($req_info['status2'] != 'S' && !($req_info['status2'] == 'D' && $w == 'u')) {
    alert("정상적인 접근이 아닙니다.", G5_URL);
}
if ($member['mb_no'] != $req_info['member_id']) {
    alert("정상적인 접근이 아닙니다.", G5_URL);
}
...
$action_url = https_url(G5_BBS_DIR)."/req_update.php";
