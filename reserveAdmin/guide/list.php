...
function guideDelete(board_no) {
	if(board_no != "") {
		if(confirm("정말로 삭제하시겠습니까?")) {
			location.href = "/admin/guide/delete_ok.php?board_no=" + board_no;
			return;
		}
	}
}
...
//-->
	...
	<div id="contentArea">
		...
		<div id="subContent">
			<div style="padding:10px 20px 10px 20px;">
				...
				<div id="insertArea">
					<input type="button" class="btn_insert" value="등록" onclick="javascript:location.href = '/admin/guide/insert.php';" />
				</div>
	...
	<?php
		$count_query = "SELECT count(*) cnt FROM guide WHERE 1=1";
		...
		$select_query = "SELECT * FROM guide WHERE 1=1";
	...
	?>
					<tr height="30px" align="center">
						<td><?=$cnt?></td>
						<td align="left"><a href="/admin/guide/modify.php?board_no=<?=$board_no?>"><?=$title?></a></td>
						<td><?=$writer?></td>
						<td><?=$reg_date?></td>
						<td><?=$hit_cnt?></td>
						<td>
							<input type="button" class="btn_submit" value="수정" onclick="location.href = '/admin/guide/modify.php?board_no=<?=$board_no?>';" />
							<input type="button" class="btn_submit" value="삭제" onclick="javascript:guideDelete('<?=$board_no?>');" //>
						</td>
					</tr>
	<?
		}
