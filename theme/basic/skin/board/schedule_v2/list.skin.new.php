...
<table width="<?=$width?>" border=0 cellpadding="0" cellspacing="0">
  <tr>
       ...
       <td width="60%" height="30" align="center">
		...
		<tr>
			<td><a href="<?php echo $_SERVER[PHP_SELF]."?bo_table=".$bo_table."&"; ?><?if ($month == 1) { $year_pre=$year-1; $month_pre=$month; } else {$year_pre=$year-1; $month_pre=$month;} echo ("year=$year_pre&month=$month_pre&sc_no=$sc_no&isNew=Y");?>">&lt;&lt;</a>&nbsp;</td>
			<td><a href="<?php echo $_SERVER[PHP_SELF]."?bo_table=".$bo_table."&"; ?><?if ($month == 1) { $year_pre=$year-1; $month_pre=12; } else {$year_pre=$year; $month_pre=$month-1;} echo ("year=$year_pre&month=$month_pre&sc_no=$sc_no&isNew=Y");?>">&lt;</a></td>
			<td style="padding:0 10px;font-size:18px;font-weight:bold;"><a href="<?php echo $_SERVER[PHP_SELF]."?bo_table=".$bo_table; ?>&isNew=Y" title="오늘로" onfocus="this.blur()"><? echo ("$year.$month"); ?></a></td>
			<td><a href="<?php echo $_SERVER[PHP_SELF]."?bo_table=".$bo_table."&"; ?><?if ($month == 12) { $year_pre=$year+1; $month_pre=1; } else {$year_pre=$year; $month_pre=$month+1;} echo ("year=$year_pre&month=$month_pre&sc_no=$sc_no&isNew=Y");?>">&gt;&gt;</a>&nbsp;</td>
			<td><a href="<?php echo $_SERVER[PHP_SELF]."?bo_table=".$bo_table."&"; ?><?if ($month == 12) { $year_pre=$year+1; $month_pre=$month; } else {$year_pre=$year+1; $month_pre=$month;} echo ("year=$year_pre&month=$month_pre&sc_no=$sc_no&isNew=Y");?>">&gt;</a></td>
		</tr>
		</table>			
	</td>
	<td width="20%" align="right">
        ...
        <ul class="btn_bo_user">
            ...
            <?php if ($write_href) { ?><li><a href="<?php echo $write_href ?>&isNew=Y" class="btn_b02"><font color="#ffffff">일정추가</font></a></li><?php } ?>
        </ul>
        <?php } ?>
</td>
  </tr>
...
<div id="bo_list">
<table width="<?=$width?>" border="0" cellspacing="0" cellpadding="5">
<thead>
  <tr bgcolor="#ffffff" align="center">     
	<th style="color:red; border:0px;">SUNDAY</th>
	<th style="border:0px;">MONDAY</th>
	<th style="border:0px;">TUESDAY</th>
	<th style="border:0px;">WEDNESDAY</th>
	<th style="border:0px;">THURSDAY</th>
	<th style="border:0px;">FRIDAY</th>
	<th style="color:blue; border:0px;">SATURDAY</th>
  </tr>
</thead>
<tbody>
<?
...
while ($row = sql_fetch_array($result)) {  // 제목글 뽑아서 링크 문자열 만들기..
  ...
  for ($i = $start_day ; $i <= $end_day;  $i++) {
    ...
    $html_day[$i].= "<br /><img src='$board_skin_url/img/".$imgown.".gif' border=0 align=absmiddle /> <a href='".G5_BBS_URL."/board.php?bo_table=$bo_table&year=$year&month=$month&wr_id=$row[wr_id]&sc_no=$sc_no&isNew=Y' id='subject_".$j."' ".$showLayer.">".$row[wr_subject]."</a>".$list[icon_new].$list[comment_cnt];
?>
    <!-- 뷰 팝업레이어 -->
...
<?
  ...
  for ($iz = 1; $iz <= $lastcount; $iz++) { // 42번을 칠하게 된다.
	...
	echo ("<td width=$col_width height=$col_height bgcolor=$bgcolor valign=top style='border:1px solid #cfcfcf;'>");
    ...
    if ($write_href) { 
      ...
      echo "<a href='$write_href&f_date=$f_date&isNew=Y'><font color='$daycolor' title='일정추가'>$daytext</font></a>$moonday <font color='$daycontcolor'>$daycont</font>$blank $annivmoonday";
  ...
  else { echo ("     <td width=$col_width height=$col_height bgcolor=f9fafe valign=top style='border:1px solid #cfcfcf;'>&nbsp;</td>"); }
  if (($iz%7) == 0) echo ("  </tr>");
